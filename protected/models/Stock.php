...
class Stock extends CActiveRecord {
    ...
    function checkStock() {
        $sql = "SELECT p.product_id,p.product_name,p.`status`,p.product_price,p.product_price_pro,p.description,IFNULL(SUM(s.total),0) as total,c.categoryname
...
INNER JOIN category c ON p.category = c.id
    ...
    function countStock($productId) {
        $sql = "SELECT IFNULL(SUM(s.total),0) AS TOTAL
                    FROM stock s
    ...
    function alertStock() {
        $sql = "SELECT COUNT(*) AS TOTAL
                FROM
                (
                        SELECT p.product_id,p.product_name,p.`status`,p.product_price,p.product_price_pro,p.description,IFNULL(SUM(s.total),0) as total,c.categoryname
                        FROM product p LEFT JOIN stock s ON p.product_id = s.product_id
                        INNER JOIN category c ON p.category = c.id
                WHERE p.`status` = '0'
                        GROUP BY p.product_id
